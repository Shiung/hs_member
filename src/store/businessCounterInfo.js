...
export default {
  ...
  actions: {
    getInfo( context, ID ){
      ...
      const url = `${process.env.API_HOST}v1/admin/businesscounter/${ID}`
      ...
      axios.get(url, {
        ...
        context.commit('BUSINESSCOUNTERINFO', res.data.data)
        ...
        if (error.response.status === 401) {
          ...
          swal({
            ...
            .then(() => {
              router.replace({name: 'businessCounter'})
            }) 
  ...
  mutations: {
    BUSINESSCOUNTERINFO ( state, payload ) {
      state.datas = payload
    }
  },
  // 運用getters 可以在其他頁面 用 mapGetters 方式 取代compute 抓取store 的方式
  getters: {
    businessCounterInfo: state => state.datas
  }
}
