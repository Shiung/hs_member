...
export default {
  ...
  actions: {
    getInfo( context, ID ){
      ...
      const url = `${process.env.API_HOST}v1/admin/content/${ID}`
      axios.get(url, {
        headers: {
          'Authorization': `${cookieToken}`
        },
      }).then((res) => {
        context.commit('NOTIFICATIONINFO', res.data.data)
  ...
  mutations: {
    NOTIFICATIONINFO ( state, payload ) {
      state.datas = payload
    }
  },
  // 運用getters 可以在其他頁面 用 mapGetters 方式 取代compute 抓取store 的方式
  getters: {
    notificationInfo: state => state.datas
  }
}
