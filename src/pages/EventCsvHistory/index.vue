...
export default {
  ...
  computed: {
    ...mapGetters('eventCsvHistoryTableMoudule', ['datatables', 'perPage', 'count', 'totalPages', 'totalCount', 'currentPage', 'paramsObj', 'paramsStatus']),
    eventID () {
  ...
  methods: {
    ...mapActions('eventCsvHistoryTableMoudule', ['getDatatable', 'getRequestParams', 'setParamsStatus']),
    refreshData (obj) {
      this.getRequestParams(obj)
      this.getDatatable(this.eventID)
      // 更新vue-table-2 一樣顯示資料
      this.$refs.eventCsvHistoryTable.setLimit(this.perPage)
    },
    prevPage () {
