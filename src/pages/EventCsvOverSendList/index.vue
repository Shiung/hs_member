...
export default {
  ...
  computed: {
    ...mapGetters('eventCsvOverSendTableMoudule', ['datatables', 'perPage', 'count', 'totalPages', 'totalCount', 'currentPage', 'paramsObj', 'paramsStatus']),
    eventID () {
  ...
  methods: {
    ...mapActions('eventCsvOverSendTableMoudule', ['getDatatable', 'getRequestParams', 'setParamsStatus']),
    refreshData (obj) {
      this.getRequestParams(obj)
      this.getDatatable(this.eventID)
      // 更新vue-table-2 一樣顯示資料
      this.$refs.eventCsvOverSendTable.setLimit(this.perPage)
    },
    prevPage () {
